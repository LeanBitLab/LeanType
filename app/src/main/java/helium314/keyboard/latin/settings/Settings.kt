...
class Settings private constructor() : SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    companion object {
        ...
        const val PREF_ANIMATION_SPEED_SCALE = "pref_animation_speed_scale"
        ...
        fun getAnimationScale(): Float = getValues().mAnimationSpeedScale

        fun getAnimationDuration(baseMillis: Long): Long {
            val scale = getValues().mAnimationSpeedScale
            if (scale <= 0.05f) return 0L
            return (baseMillis * scale).toLong()
        }
